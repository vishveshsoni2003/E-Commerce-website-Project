  ...
  return productData ?  (
    <div className='border-t-2 pt-10 transition-opacity ease-in duration-500 opacity-100'>
         ...
         <div className='flex flex-col gap-4 my-8'>
          ...
          <div className='flex gap-2'>
              {productData.sizes.map((item, index) => (
                <button onClick={() => setSize(item)} className={`border py-2 px-4 bg-gray-100 cursor-pointer ${item == size ? 'border-orange-500' : 'border-gray-100'}`} key={index}>{item}</button>
              ))}
          </div>
         </div>
      ...
      <div className='flex flex-col gap-4 border px-6 text-sm text-gray-500'>
        <p>Lorem ipsum, dolor sit amet consectetur adipisicing elit. Dicta, repudiandae. Iure eligendi cum commodi quod laudantium, excepturi suscipit optio eaque hic quidem in qui? Consequuntur reiciendis odit dignissimos inventore magni.</p>
