...
const syncCartToDatabase = async (token) => {
    try {
        // Get current local cart
        const localCart = JSON.parse(localStorage.getItem('cartItems') || '{}');
        
        if (Object.keys(localCart).length === 0) {
            // No local cart, just fetch from database
            await getUserCart(token);
            return;
        }
        
        // Sync each item in local cart to database
        ...
        // Clear local cart from localStorage after syncing
        localStorage.removeItem('cartItems');
        
        // Fetch the complete cart from database
        await getUserCart(token);
    ...
    // Fetch cart from database when token is set
    useEffect(()=>{
        if(token){
            syncCartToDatabase(token)
        }
    },[token])
